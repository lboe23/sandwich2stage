---
title: "Sandwich Variance in 2 Stage Regression with Data from a Simple Random Sample (SRS)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Sandwich Variance in 2 Stage Regression with Data from a Simple Random Sample (SRS)}
...
First, we load the data from our package. Note that this sample data set is assumed to be from a simple random sample and has 10,000 rows (e.g. subjects) and 9 columns:
- $ID$: a unique ID variable for each subject in the data frame
- $x$: the true continuous exposure variable of interest (usually unknown)
...
head(sandwichdata_SRS)
```

Next, we will create the survey design object, choosing a simple random sample for our survey design. 
```{svy design}
